...
Внутри Klipper сегмент регистрируется с одним ведущим `_` (скрытие в UI).
Если секция уже названа `_heater_bed0`, второй префикс не добавляется.
`verify_heater` должен совпадать с этим именем: `[verify_heater _heater_bed0]`.
